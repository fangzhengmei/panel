...
> **关键洞察（经代码逐行验证）**：
> 1. `resources` 路由在**任何异常状态下都对所有角色拦截**——不存在"暂停时放行"或"安装时仅管理员放行"
> 2. 暂停/维护状态下，`ws` 路由在**第一个 `if`** 就被拦截（`isSuspended()=true && !routeIs('resources')=true`），根本走不到第二个 `if` 的管理员例外逻辑
> 3. 只有 `view` 路由在任何异常状态下都对所有角色放行——注释写着 "Still allow users to get information about their server"
> 4. 只有在安装/迁移/恢复备份状态下，且路由是 `ws`，且用户是管理员，三个条件同时满足时才放行
...
if (!$request->routeIs('view')) {
    // if 1: 暂停/维护 且 非 resources → throw
    //   暂停且 resources → 不 throw，继续执行 if 2
    //   安装/迁移 → 不 throw，继续执行 if 2
    if (($isSuspended || isUnderMaintenance) && !routeIs('resources')) {
        throw;
    }
    // if 2: 非管理员 或 非 ws → throw
    //   管理员 + ws → 不 throw → ✅ 放行
    //   管理员 + resources → throw（!routeIs('ws') = true）
    //   非管理员 + ws → throw（!root_admin = true）
    if (!$root_admin || !routeIs('ws')) {
        throw;
    }
}
```
